...
//Impostazioni stampa
$stampa_ore_notturne = $impostazioni_modulo["impostazioni_hr_stampa_ore_notturne"] ?? DB_BOOL_FALSE;
$stampa_reperibilita = $impostazioni_modulo["impostazioni_hr_stampa_reperibilita"] ?? DB_BOOL_FALSE;
$stampa_trasferte = $impostazioni_modulo["impostazioni_hr_stampa_trasferte"] ?? DB_BOOL_FALSE;
$stampa_buono_pasto = $impostazioni_modulo["impostazioni_hr_stampa_buono_pasto"] ?? DB_BOOL_FALSE;
$stampa_smartworking = $impostazioni_modulo["impostazioni_hr_stampa_smartworking"] ?? DB_BOOL_FALSE;
$stampa_anomalie = $impostazioni_modulo["impostazioni_hr_stampa_anomalie"] ?? DB_BOOL_FALSE;
$stampa_spese = $impostazioni_modulo["impostazioni_hr_stampa_spese"] ?? DB_BOOL_FALSE;
...
// 26/08/2024 - Commentate voci busta per stampa
$voci_busta = [
    ...
    //['key' => 'buono_pasto', 'name' => 'Indennità mensa'],
...
array_splice($voci_busta, 1, 0, $elemento_aggiungere); // splice in at position 3

// 26/08/2024 - Settings per stampa
if ($stampa_ore_notturne) {
    $voci_busta[] = ['key' => 'notturno', 'name' => 'Ore Notturne'];
}
if ($stampa_trasferte && $impostazioni_modulo['impostazioni_hr_reperibilita'] == 1) {
    $voci_busta[] = ['key' => 'reperibile', 'name' => 'Reperibilità'];
}
if ($stampa_trasferte) {
    $voci_busta[] = ['key' => 'trasferta', 'name' => 'Trasferte'];
}
if ($stampa_buono_pasto) {
    $voci_busta[] = ['key' => 'buono_pasto', 'name' => 'Buono pasto'];
}
if ($stampa_smartworking) {
    $voci_busta[] = ['key' => 'smartworking', 'name' => 'Smartworking'];
}
if ($stampa_anomalie) {
    $voci_busta[] = ['key' => 'anomalia', 'name' => 'Anomalia'];
}

/* $settings = $this->apilib->searchFirst('settings');
$image = !empty($settings['settings_company_logo']) ? $settings['settings_company_logo'] : null; */
?>
    ...
    <tbody>
        <thead>
            ...
            <tr>
                ...
                <th>Giorni</th>
                <th>Ore</th>
                <?php if ($stampa_reperibilita && $impostazioni_modulo['impostazioni_hr_reperibilita'] == 1): ?>
                    <th>GG reperibili</th>
                <?php endif; ?>
                <th>Ore assenza</th>
                <?php if ($stampa_spese) : ?>
    ...
    <tbody>
        ...
        <?php foreach ($voci_busta as $key => $voce_busta): ?>
            <tr>
                ...
                <!-- Colonna per Giorni Totali -->
                <td>
                    <?php
                    // Mostra i giorni totali in base alla voce corrente
                    if ($voce_busta['key'] == 'ordinarie') {
                        echo $presenze['conteggi']['giorni_ordinari'];
                    } elseif ($voce_busta['key'] == 'notturno') {
                        echo $presenze['conteggi']['giorni_notturni'];
                    } elseif ($voce_busta['key'] == 'straordinarie' && $impostazioni_modulo['impostazioni_hr_banca_ore'] == 0) {
                        echo $presenze['conteggi']['giorni_straord'];
                    } elseif ($voce_busta['key'] == 'straordinarie' && $impostazioni_modulo['impostazioni_hr_banca_ore'] == 1) {
                        echo $presenze['conteggi']['banca'];
                    }
                    ?>
                </td>
                <!-- Colonna per Ore Totali -->
                <td>
                    <?php
                    // Mostra le ore totali in base alla voce corrente
                    if ($voce_busta['key'] == 'ordinarie') {
                        echo $presenze['conteggi']['ore_ordinarie'];
                    } elseif ($voce_busta['key'] == 'notturno') {
                        echo $presenze['conteggi']['ore_notturno'];
                    } elseif ($voce_busta['key'] == 'straordinarie' && $impostazioni_modulo['impostazioni_hr_banca_ore'] == 0) {
                        echo $presenze['conteggi']['ore_straord'];
                    } elseif ($voce_busta['key'] == 'straordinarie' && $impostazioni_modulo['impostazioni_hr_banca_ore'] == 1) {
                        echo $presenze['conteggi']['banca'];
                    }
                    ?>
