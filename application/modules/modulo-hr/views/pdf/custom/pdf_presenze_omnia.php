<?php
    ...
    foreach ($dipendenti as $dipendente) {
        ...
        for ($giorno = 1; $giorno <= $giorni_mese; $giorno++) {
            ...
            //CONTROLLO NOTE SPESA GIORNO CORRENTE, può non essere legata ad una presenza
            $this->db->select('*');
            $this->db->from('nota_spese');
            $this->db->where("DATE_FORMAT(nota_spese_data, '%Y-%m-%d') = '{$current_date}'", null, false);
            $this->db->where('nota_spese_dipendente', $dipendente['dipendenti_id']);
            $note_spese_dip_giorno = $this->db->get()->result_array();
            
            $spese_tot = 0;
            if(!empty($note_spese_dip_giorno)) {
                foreach($note_spese_dip_giorno as $nota) {
                    $spese_tot += $nota['nota_spese_importo'];
                }
                $dati_dip[$giorno]['note_spesa'] = number_format($spese_tot, 2, ',', '.');
            }
                ...
                foreach ($pres as $p) {
                    if (date('d', strtotime($p['presenze_data_inizio'])) == $giorno) {
                        ...
                        if (!empty($p['presenze_notturno']) && (float) $p['presenze_notturno'] > 0 && (float) is_numeric($p['presenze_notturno'])) {
                            ...
                            $conteggi['ore_ordinarie'] -= $p['presenze_notturno'];
                            $dati_dip[$giorno]['ordinarie'] -= $p['presenze_notturno'];
