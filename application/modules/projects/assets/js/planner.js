...
var Planner = function () {
    ...
    return {
        ...
        setTaskcolumn: function (taskId, newcolumn) {
            $.post(base_url + 'firecrm/main/editTask/' + taskId, {
                tasks_status: newcolumn,
        ...
        init: function () {
            ...
            $('[data-task] .task-body .task-title').on('click', function () {
                if (dragging) {
                    return false;
                }
                var taskPortlet = $(this).parents('[data-task]');
                var id = taskPortlet.attr('data-task');
                loadModal(base_url + 'get_ajax/layout_modal/task-detail/' + id + '?_size=extra');

            });

            $('[data-task] .task-content').on('click', function () {
                if (dragging) {
                    return false;
                }
                var taskPortlet = $(this).parents('[data-task]');
