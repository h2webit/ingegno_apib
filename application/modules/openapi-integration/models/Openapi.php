...
class Openapi extends CI_Model
{
    ...
    // +++++++++++++++++++++++++++++++++++++ NUOVO METODO GENERICO PER RICHIESTE +++++++++++++++++++++++++++++++++++++++++++++


    // Metodo generico che sfrutta i settaggi di un servizio per effettuare chiamata di post, addebito e ritornare l'output della chiamata.
    // Comodo per servizi generici come cerca cliente etc..
    ...
    // Todo: quando invalidare la cache?
    public function get_from_cache($servizio_id, $url, $post = null)
    {
        $result = $this->db->where('openapi_cache_url', $url)->where('openapi_cache_servizio', $servizio_id)->where('openapi_cache_post', $post)->limit(1)->order_by('openapi_cache_id DESC')->get('openapi_cache')->row_array();

        if (!empty($result)) {
            return $result['openapi_cache_output'];
        } else {
            return false;
        }
    }
    ...
    public function addebito($importo, $servizio = null, $servizio_id = null)
    {
        ...
        if ($this->openapi_settings['openapi_settings_saldo'] >= $importo) {
            // Saldo su settings - DEPRECATO PER PASSAGGIO A PP
    ...
    // Funzione inutilizzata per ora. Esiste PP Che incrementa saldo in caso di movimento di Accredito.
    ...
    public function ricerca_tabulati_imprese($ricerca, $id_servizio)
    {
        ...
        if (!empty($output)) {
            ...
            if ($output['success'] !== false) {
                ...
                // Save to cache
                $this->openapi->save_to_cache($id_servizio, $url, json_encode($parametri), $json_output);
                ...
                if ($servizio['openapi_servizi_costo_chiamata'] > 0) {
                    ...
                    $this->addebito($costo, $servizio['openapi_servizi_nome'], $servizio['openapi_servizi_id']);
                }
                // Salvo i dati
    ...
    // Cerca automezzo da targa
    public function cerca_automezzo($targa)
    {
        $id_servizio = 20; // TODO: Soluzione non definitiva
        $servizio = $this->db->query("SELECT * FROM openapi_servizi WHERE openapi_servizi_id = '$id_servizio'")->row_array();
        ...
        if ($output['success'] !== false) {
            ...
            // Salvo il dato grezzo anche in caso di esito negativo
            $this->openapi->save_to_cache($id_servizio, $url, $targa, $json_output);
        }
