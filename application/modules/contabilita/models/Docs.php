...
class Docs extends CI_Model
{
    ...
    public function doc_express_save(array $data = [])
    {
        ...
        if (!empty($data['scadenze'])) {
            ...
            $template_scadenza = $this->apilib->view('documenti_contabilita_template_pagamenti', $template_pagamento);

            // Riordino le sotto scadenze sul campo "giorni"
            usort($template_scadenza['documenti_contabilita_tpl_pag_scadenze'], function ($a, $b) {
            ...
            foreach ($template_scadenza['documenti_contabilita_tpl_pag_scadenze'] as $index => $tpl_pag_scadenza) {
                // Creo l'oggetto datetime partendo dalla data scadenza passata in post (se presente, altrimenti data emissione)
                ...
                if (!empty($tpl_pag_scadenza['documenti_contabilita_tpl_pag_scadenze_giorni'])) {
                    switch ($tpl_pag_scadenza['documenti_contabilita_tpl_pag_scadenze_giorni']) {
                        ...
                        case '90':
                        case '120':
                            // Aggiungo i mesi in base ai giorni specificati
                            $mesiDaAggiungere = (int) ($tpl_pag_scadenza['documenti_contabilita_tpl_pag_scadenze_giorni'] / 30);
                            $dataScadenzaBaseObj->modify("+{$mesiDaAggiungere} months");
                            break;
                        default:
                ...
                switch ($tpl_pag_scadenza['documenti_contabilita_tpl_pag_scadenze_tipo_value']) {
                    case 'Fine mese':
                        // Imposto la data alla fine del mese corrente
                        $dataScadenzaBaseObj->modify('last day of this month');
                        break;
                    case 'Data Fattura':
                        // Qui non faccio nulla poiché la data è già impostata
                        break;
                    default:
                ...
                if (($mese_escluso_1 > 0 && $dataScadenzaBaseObj->format('n') == $mese_escluso_1) || ($mese_escluso_2 > 0 && $dataScadenzaBaseObj->format('n') == $mese_escluso_2)) {
                    if (!empty($giorni_spostamento) && $giorni_spostamento > 0) {
                        $dataScadenzaBaseObj->modify("+{$giorni_spostamento} days");
                    }
                }
