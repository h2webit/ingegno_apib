...
class RibaAbiCbi extends CI_Model
{
    ...
    public function creaFileFromDocumenti($settings, $_scadenze, $accorpata = false)
    {
        ...
        $intestazione = [
            ...
            //         [7] = ragione_soc2_creditore variabile lunghezza 24 alfanumerico
            8 => $settings['documenti_contabilita_settings_company_address'],
            //         [8] = indirizzo_creditore variabile lunghezza 24 alfanumerico
        ...
        foreach ($scadenze as $documento) {
            ...
            //     $dest_iban_data = $this->extractIbanData($dest['customers_bank_account_iban']);
            // } elseif (!empty($dest['customers_bank_account_abi']) && !empty($dest['customers_bank_account_cab']) && !empty($dest['customers_bank_account_cin']) && !empty($dest['customers_bank_accounts_account_number'])) {
            //     $dest_iban_data = "IT{$dest['customers_bank_account_cin']}{$dest['customers_bank_account_abi']}{$dest['customers_bank_account_cab']}{$dest['customers_bank_accounts_account_number']}";
            // } else {
            ...
            //     } else {
            //         if ($banca['customers_bank_accounts_iban']) {
